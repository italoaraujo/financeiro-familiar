    ...
    <AppShell>
      <div className="space-y-8">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <input
                ...
                className="bg-slate-900 border border-slate-700/80 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium cursor-pointer"
              />
            </div>
            <Link
              href="/transactions"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-slate-950 font-semibold px-4 py-2 rounded-xl text-sm shadow-lg shadow-emerald-500/20 transition-all"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {/* Total Balance */}
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group hover:border-slate-700 transition-all">
                ...
                <div className="mt-4 space-y-2 max-h-40 overflow-y-auto pr-1">
                  {categoryExpenses.map((cat, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-slate-400 font-semibold">{formatCurrency(cat.amount)} ({cat.percentage}%)</span>
                    </div>
                  ))}
                </div>
              </>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-white">Últimos Lançamentos</h3>
            ...
            {summary?.recentTransactions?.length === 0 ? (
              <p className="text-sm text-slate-500 py-6 text-center">Nenhuma transação recente encontrada.</p>
            ) : (
          ...
          <div className="lg:col-span-5 bg-slate-900/80 border border-slate-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-bold text-white">Faturas em Aberto</h3>
            ...
            {summary?.openInvoices?.length === 0 ? (
              <p className="text-sm text-slate-500 py-6 text-center">Nenhuma fatura em aberto no momento.</p>
            ) : (
              <div className="space-y-3">
                {summary?.openInvoices?.map((inv: any) => (
                  <div
                    key={inv.id}
                    className="p-4 bg-slate-800/50 border border-slate-700/60 rounded-xl flex items-center justify-between"
                  >
                    <div className="flex items-center gap-3">
                      <CreditCard className="h-6 w-6 text-purple-400" />
                      <div>
                        <p className="text-sm font-semibold text-white">{inv.creditCard?.name}</p>
                        <p className="text-xs text-slate-400">
